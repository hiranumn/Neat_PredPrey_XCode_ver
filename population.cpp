...
bool Population::epoch(int generation) {
    ...
    // either call tournamentSelect or the while loop below, but not both
    tournamentSelect(generation);
...
/*
 * Do tournament selection on the population.
 *
 * Added to NEAT by Alby Himelick, 8/12/13
 */
void Population::tournamentSelect(int generation) {
    vector<Organism*> neworgs;
    
    vector<Organism*> tourny;
    Organism* bestorg;
    double bestfit;
    Genome* new_genome;
    Organism* neworg;
    ...
    for (int i = 0; i < NEAT::pop_size; ++i) {
        // randomly select organisms for a tournament
        tourny.clear();
        int num_selected = 0;
        int num_seen = 0;
        double prob;
        for (vector<Organism*>::iterator curorg = organisms.begin(); curorg != organisms.end(); ++curorg) {
            prob = (tournament_size - num_selected) / (NEAT::pop_size - num_seen);
            if (prob == 0) break;
            if (randfloat() < prob) {
                tourny.push_back(*curorg);
                ++num_selected;
            }
            ++num_seen;
        }
        ...
        bestorg = *(tourny.begin());
        bestfit = bestorg->fitness;
        for (vector<Organism*>::iterator curorg = tourny.begin(); curorg != tourny.end(); ++curorg) {
            if ((*curorg)->fitness > bestfit) {
                bestorg = *curorg;
                bestfit = bestorg->fitness;
            }
        }
        
        // build copy of best organism
        new_genome = (bestorg->gnome)->duplicate(i);
        neworg = new Organism(0.0, new_genome, generation);
        
        // tell the copy what species it should belong to
        neworg->species = bestorg->species;
        neworgs.push_back(neworg);
    }
    
    // empty all species of old organisms
    for (vector<Species*>::iterator curspecies = species.begin(); curspecies != species.end(); ++curspecies) {
        while (!(*curspecies)->organisms.empty()) {
            Organism* org = (*curspecies)->organisms.back();
            (*curspecies)->organisms.pop_back();
            delete org;
        }
    }
    
    // actually add new organisms to their species
    for (vector<Organism*>::iterator curorg = neworgs.begin(); curorg != neworgs.end(); ++curorg) {
        (*curorg)->species->add_Organism(*curorg);
    }
    
    // modify the master population list
    organisms.swap(neworgs);
}
